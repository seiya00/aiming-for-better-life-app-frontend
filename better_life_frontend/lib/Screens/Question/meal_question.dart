...
class MealQuestion extends StatefulWidget {
  const MealQuestion({super.key});

  @override
  State<MealQuestion> createState() => _MealQuestionState();
}

class _MealQuestionState extends State<MealQuestion> {
  bool? isFinishedMealQuestion;
  ...
  void initState() {
    ...
    getQuestions().then((fetchedData) {
      setState(() {
        ...
        mealQuestionsList =
            fetchedData.where((map) => map['question_type'] == '食事').toList();
  ...
  void _nextQuestion(inputData, BuildContext context) async {
    ...
    setState(() {
      if (currentQuestionIndex < mealQuestionsList.length - 1) {
        ...
        globalState.setIsFinishedMealQuestion(true);
        Navigator.push(
          context,
