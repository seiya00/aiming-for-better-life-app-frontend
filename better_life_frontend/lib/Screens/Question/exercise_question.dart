...
class ExerciseQuestion extends StatefulWidget {
  const ExerciseQuestion({super.key});

  @override
  State<ExerciseQuestion> createState() => _ExerciseQuestionState();
}
...
class _ExerciseQuestionState extends State<ExerciseQuestion> {
  ...
  void initState() {
    super.initState();
    isLoading = true;
    _getQuestions().then((fetchedData) {
      setState(() {
        currentQuestionIndex = 0;
        mealQuestionsList =
            fetchedData.where((map) => map['question_type'] == '運動').toList();
  ...
  void _nextQuestion(inputData) async {
